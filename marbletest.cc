...
void setup_shaders() {
    ...
    // pass world coordinate to geometry shader
    // transform world coordinate to screen position
...
rp3d::Vector3 SPAWN_POS2(0.0, 2.5, -0.1);
...

{
    // marble sphere 2
    //rp3d::Transform marble_pose(rp3d::Vector3(-0.5, 3.5, -1.0), rp3d::Quaternion::identity());
    rp3d::Transform marble_pose(SPAWN_POS2, rp3d::Quaternion::identity());
    marble2_body = world->createRigidBody(marble_pose);
    auto marble_shape = new rp3d::SphereShape(0.5);
    marble2_body->addCollisionShape(marble_shape, rp3d::Transform(), 1.0);
    auto marble_mat = marble2_body->getMaterial();
    marble_mat.setBounciness(rp3d::decimal(0.5));
    marble_mat.setFrictionCoefficient(rp3d::decimal(0.5));
    marble2_body->setMaterial(marble_mat);
}
...
void physics_step(float dt) {
    ...
    // check for fall out 2
{
    rp3d::Transform t = marble2_body->getTransform();
    rp3d::Vector3 pos = t.getPosition();
    if (pos.y < -OUT || abs(pos.x) > OUT || abs(pos.y) > OUT) {
        t.setPosition(SPAWN_POS);
        marble2_body->setTransform(t);
        marble2_body->setLinearVelocity(rp3d::Vector3(0,0,0));
    }
}
}
...
void draw_scene() {
    ...
    */
    ...
    // marble sphere 2 marble boogaloo
    auto marble2_tran = marble2_body->getTransform();
    glm::vec3 marble2_pos = xlat(marble2_tran.getPosition());
{
    // set up uniform values
    glUseProgram(marbleShaderProgram);
    unsigned int projectionLoc = glGetUniformLocation(marbleShaderProgram, "projection");
    ...
    //cout << marble2_pos.x << ',' << marble2_pos.y << ',' << marble2_pos.z << endl;
    unsigned int obj_posLoc = glGetUniformLocation(marbleShaderProgram, "obj_pos");
    glUniform3fv(obj_posLoc, 1, glm::value_ptr(marble2_pos));
    ...
    auto model = glm::mat4(1.0f);
    model = glm::translate(model, marble2_pos);
    unsigned int modelLoc = glGetUniformLocation(marbleShaderProgram, "model");
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

    rp3d::Matrix3x3 tempm = marble2_tran.getOrientation().getMatrix();
    glm::mat3 marble_rot;
    for (int y=0 ; y<3 ; y+=1) {
        for (int x=0 ; x<3 ; x+=1) {
            marble_rot[y][x] = tempm[y][x];
    ...
    */
    ...
    unsigned int sphere2_posLoc = glGetUniformLocation(groundShaderProgram, "sphere2_pos");
    glUniform3fv(sphere2_posLoc, 1, glm::value_ptr(marble2_pos));
    unsigned int sphere2_rLoc = glGetUniformLocation(groundShaderProgram, "sphere2_r");
    glUniform1f(sphere2_rLoc, 0.5);
